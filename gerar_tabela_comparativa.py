...
def generate_comparative_latex_table(best_results, k, top_words, base, exp_num, npmi_info):
    """Gera o código LaTeX da tabela comparativa contendo os melhores resultados."""
    ...
    # Inicia a tabela LaTeX
    # Como serão 4 colunas de algoritmos + 1 coluna de índice, usamos \tiny para caber na página
    cols_format = " ".join(["X"] * len(ALGORITMOS))
    latex_lines = [
        "\\begin{table}[htpb]",
        "    \\centering",
        "    \\tiny % Fonte pequena recomendada para tabelas com muitas colunas",
        f"    \\begin{{tabularx}}{{\\textwidth}}{{@{{}}l {cols_format}@{{}}}}",
        "        \\toprule"
    ]
    ...
    for tid in sorted_keys:
        ...
        row_items = [f"\\textbf{{{label}}}"]
        
        # Para cada algoritmo, pega as palavras do tópico (ou coloca um traço se não existir, ex: tópico -1 no LDA)
    ...
    info_str = ", ".join([f"{algo.upper()} (Exp {npmi_info[algo]['letra']}, NPMI: {npmi_info[algo]['npmi']:.4f})" 
                          for algo in ALGORITMOS if npmi_info.get(algo)])
                          
    caption = (f"Comparativo dos tópicos extraídos para a base {base} ($K={k}$, Top {top_words} palavras). "
    ...
    latex_lines.extend([
        ...
        "    \\end{tabularx}",
        f"    \\caption{{{caption}}}",
        f"    \\label{{tab:comparativo_{base}_exp{exp_num}_k{k}}}",
        "\\end{table}\n"
    ])
